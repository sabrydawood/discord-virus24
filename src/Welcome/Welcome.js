...
constructor() {
  ...
  this.textWelcome = "Welcome To {guild}";
  //add on/of methods 
this.addonBackgroundImage = true;
  // can changed colors 
    ...
    this.colorWelcomeBox = "pink";
    this.colorWelcomeText = "#fff";
...
//math numbers above 1000
  if (this.members >= 1000) {
   ...
   this.bots = math(this.bots, 1)
    ...
    if(this.textWelcome.length > 100 || typeof this.textWelcome !== 'string' ){
    error("The Welcome Text Must Be A String  And __Less__ Than `100` Characters")
  }
}
...
// members info 
  ...
  setText(variable, value) {
    ...
    if (this[formattedVariable]) this[formattedVariable] = value;
  if(this.textWelcome.length >= 100){
    error("The GodBye Text Must Be  (Less Than)<<=[100] Characters")
  }else if(typeof this.textWelcome !== 'string'){
...
let welcomeText = this.textWelcome.replace(/{guild}/g, this.welcomeText);
...
if (this.addonBackgroundImage) {
 ...
 ctx.drawImage(background,25, 20, canvas.width - 50, canvas.height - 380);
   //roundImage(canvas,25, 20, canvas.width - 50, canvas.height - 380, 30, 5, background)
    
}
    // Welcome rectangle
ctx.globalAlpha = 1;
ctx.fillRect(roundRect(canvas,165, 100, 750, 100, 20, 4,"aqua", this.colorWecomeBox));
ctx.globalAlpha = 1;

      // Welcome text
    ctx.textAlign = "left";
    ctx.fillStyle =this.colorWelcomeText;
